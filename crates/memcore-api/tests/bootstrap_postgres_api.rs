...
#[tokio::test]
async fn missing_postgres_url_fails_startup() {
...
#[tokio::test]
async fn postgres_fact_backend_attempts_connection_when_url_present() {
    let settings = Settings {
        fact_backend: FactBackend::Postgres,
...
async fn postgres_fact_backend_starts_with_real_database() {
    ...
    AppState::initialize(settings)
        .await
        .expect("postgres fact backend should initialize with test database");
}
